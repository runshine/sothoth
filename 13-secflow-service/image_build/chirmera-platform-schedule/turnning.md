...
当 `chirmera-platform-schedule` 的“任务-任务中心”创建 `AI4APK` 任务时，传给本服务的 `file_path` 固定表示“待扫描目录路径”，而不是单个 APK/HAP 文件路径。

固定语义如下：

- 调度中心只负责把用户在项目文件系统中选中的目录绝对路径原样传入 `file_path`
- AI4APK 服务负责在该目录内自行发现 APK/HAP 文件
- APK/HAP 的搜索、唯一性判定、递归规则以及错误提示均由 AI4APK 服务负责

因此，本接口虽然保留 `file_path` 这个字段名，但对任务中心接入场景，字段语义应理解为“扫描根目录”
