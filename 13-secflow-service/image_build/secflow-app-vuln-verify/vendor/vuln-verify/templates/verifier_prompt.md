...
- 遇到无法确定的情况，判为 unresolved 并记录原因。
- `dimensions` 四个维度必须全部分析，不能跳过。
...
- `null` — 静态分析无法确定（仅 unresolved 时出现）
- verdict=confirmed → 四个 status 都是 true，ruled_out_by=null
- verdict=ruled_out → 恰好一个 status 是 false，ruled_out_by 指向该维度
- verdict=unresolved → 至少一个 status 是 null，ruled_out_by=null
