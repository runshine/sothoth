...
class TaskRuntimeServiceMixin:
    def _runtime_lease_log_view(self: TaskManager, db: Session | None, task) -> tuple[str | None, str | None]:
        ...
        owner = str(getattr(lease, "owner_instance_id", "") or "").strip() or None if lease is not None else None
        expires_at = (
            task_manager_module._isoformat_or_none(getattr(lease, "lease_expires_at", None))
    ...
    async def _drop_unclaimed_dispatch_task_after_pop(
        ...
        *,
        ...
        if task is not None:
            ...
            owner_runtime_supported = bool(snapshot.runtime_lease_active and snapshot.runtime_lease_owner)
            self._record_event(
                db,
                task,
            ...
            task_manager_module.logger.warning(
                ...
                "runtime_lease_active=%s runtime_lease_owner=%s local_handle_alive=%s",
                normalized_task_id,
                normalized_reason,
                task_status,
                runtime_phase,
    ...
    async def _cooldown_unclaimed_dispatch_task_after_pop(
        ...
        *,
        ...
        task_manager_module.logger.warning(
            ...
            "task_id=%s reason=%s cooldown_seconds=%s status=%s runtime_phase=%s",
            normalized_task_id,
            normalized_reason,
            int(cooldown_seconds or 0),
    ...
    async def _requeue_unclaimed_dispatch_task(self: TaskManager, db: Session, task_id: str) -> None:
        ...
        if task is not None:
            ...
            self._record_event(
                db,
                task,
                "delete_queue_signal_reconciled" if active_delete_operation is not None else "dispatch_claim_reenqueued",
                reenqueue_message,
                level="info" if active_delete_operation is not None else "warning",
                stage_name=task.current_stage,
                payload={
                    "task_id": normalized_task_id,
                    ...
                    "signal_channel": "delete_queue" if active_delete_operation is not None else "shared_dispatch",
                },
            )
            db.commit()
    ...
    def _normalize_delete_queue_task_state(
        self: TaskManager,
        db: Session,
        task,
        *,
        ...
        stale_owner_cleared = False
        runtime_phase_repaired = False
        hidden_state_repaired = False
        if operation is not None and not self._task_is_hidden_by_delete_queue(task):
            self._mark_task_delete_queued(task, operation_id=operation.id, force_delete=force_delete)
            hidden_state_repaired = True
        if not runtime_lease_active:
            if runtime_lease is not None:
                self._clear_runtime_lease(db, task.id)
                stale_owner_cleared = True
            task_status = str(getattr(task, "status", "") or "").strip().lower()
            if task_status in task_manager_module.TASK_TERMINAL_STATUSES and self._task_runtime_phase(task) != task_manager_module.TASK_RUNTIME_PHASE_TERMINAL:
                self._set_task_runtime_phase(task, task_manager_module.TASK_RUNTIME_PHASE_TERMINAL)
                runtime_phase_repaired = True
        if stale_owner_cleared or runtime_phase_repaired or hidden_state_repaired:
            self._record_event(
                db,
                task,
                "delete_queue_task_state_normalized",
                "delete queue 消费前已修复陈旧 owner/隐藏态元数据",
                level="info",
                stage_name=str(getattr(task, "current_stage", "") or "").strip() or None,
                payload={
                    "operation_id": operation_id,
                    "stale_owner_cleared": stale_owner_cleared,
                    "runtime_phase_repaired": runtime_phase_repaired,
                    "hidden_state_repaired": hidden_state_repaired,
                    "runtime_phase": self._task_runtime_phase(task),
                    "task_status": str(getattr(task, "status", "") or "").strip() or None,
                },
            )
    ...
    async def _ensure_delete_queue_signal(
        self: TaskManager,
        db: Session,
        task,
        queue,
        delete_positions: dict[str, int],
        *,
        enqueue_context: str,
    ) -> bool:
        active_delete_operation = self._active_delete_queue_operation(db, task)
        if active_delete_operation is None:
            if self._task_is_hidden_by_delete_queue(task):
                self._clear_stale_delete_queue_hidden_state(
                    db,
                    task,
                    reason="delete_queue_signal_missing_active_delete_operation",
                )
                return True
            return False
        force_delete = bool(dict(getattr(active_delete_operation, "request_payload", {}) or {}).get("force_delete"))
        if not self._task_is_hidden_by_delete_queue(task):
            self._mark_task_delete_queued(task, operation_id=active_delete_operation.id, force_delete=force_delete)
        task_id = str(getattr(task, "id", "") or "").strip()
        ...
        self._record_event(
            db,
            task,
            "delete_queue_signal_reconciled",
            "delete queue 信号缺失，已补回 Redis delete queue",
            level="info",
            stage_name=str(getattr(task, "current_stage", "") or "").strip() or None,
            payload={
                "task_id": task_id,
    ...
    async def _consume_delete_queue_task(self: TaskManager, db: Session, task_id: str) -> None:
        ...
        if task is None:
            return
        active_operation = self._active_delete_queue_operation(db, task)
        if active_operation is None:
            stale_hidden_state_cleared = False
            if self._task_is_hidden_by_delete_queue(task):
                stale_hidden_state_cleared = self._clear_stale_delete_queue_hidden_state(
                    db,
                    task,
                    reason="delete_queue_consumer_missing_active_delete_operation",
                )
            self._record_event(
                db,
                task,
                ...
                stage_name=task.current_stage,
                level="info",
                payload={"task_id": task.id, "stale_hidden_state_cleared": stale_hidden_state_cleared},
            )
            db.commit()
            return
        force_delete = bool(dict(active_operation.request_payload or {}).get("force_delete"))
        normalize_result = self._normalize_delete_queue_task_state(
            db,
            task,
        ...
        self._record_event(
            ...
            payload={
                ...
                "owner_released_before_delete_consume": bool(normalize_result.get("stale_owner_cleared")),
                "state_normalized_before_delete_consume": bool(
                    normalize_result.get("stale_owner_cleared")
                    or normalize_result.get("runtime_phase_repaired")
                    or normalize_result.get("hidden_state_repaired")
                ),
                "decision_reason_code": delete_takeover_decision.reason_code,
            },
        )
        db.commit()
        try:
    ...
    def _run_parent_reclaim_pass(
        self: TaskManager,
        db: Session,
        *,
        include_owned_execution_takeover_requeue: bool = True,
    ):
        from app.service import task_manager as task_manager_module

        def _owned_execution_takeover_requeue() -> bool:
            if not include_owned_execution_takeover_requeue:
                return False
            return self._requeue_orphaned_owned_execution_locked(db)
        ...
        if not isinstance(db, Session):
            return (
                ...
                _owned_execution_takeover_requeue(),
    ...
    def _owned_execution_requeue_candidate_task_ids(self: TaskManager, db: Session) -> list[str]:
        from app.service import task_manager as task_manager_module
    ...
    def _stale_dispatching_candidate_task_ids(self: TaskManager, db: Session) -> list[str]:
        ...
        rows = (
            db.query(task_manager_module.BinarySecurityTask.id)
            .filter(task_manager_module.BinarySecurityTask.status == "dispatching")
            .all()
        )
        return [
            str(task_id or "").strip()
    ...
    async def _reconcile_work_queues_once(self: TaskManager, db: Session, *, now_value: datetime | None = None) -> None:
                ...
                if current_status == "pending":
                    ...
                    self._record_event(
                        ...
                        payload={
                            ...
                            "queue_state": "db_pending_not_enqueued",
                            "recoverable_reason": "pending_task_delete_queue_authoritative",
                            "runtime_lease_owner": runtime_lease_owner,
                            "runtime_lease_expires_at": runtime_lease_expires_at,
                            "current_operation_id": str(getattr(task, "current_operation_id", "") or "").strip() or None,
            ...
            if current_status == "pending":
                ...
                if queue_state == "db_pending_not_enqueued":
                    ...
                    task_manager_module.logger.warning(
                        ...
                        "recoverable_reason=%s runtime_lease_owner=%s runtime_lease_expires_at=%s current_operation_id=%s",
                        normalized_task_id,
                        current_status,
                        queue_state,
                        recoverable_reason,
                        runtime_lease_owner,
                        task_manager_module._isoformat_or_none(getattr(runtime_lease, "lease_expires_at", None))
                        if runtime_lease is not None
                        else None,
        ...
        for (operation_task_id,) in operation_rows:
            ...
            if task is None:
                continue
            if await self._ensure_delete_queue_signal(
                db,
                task,
                queue,
                delete_positions,
                enqueue_context="delete_queue_reconcile_operation",
            ):
    ...
    def _dispatch_task_by_id(self: TaskManager, db: Session, task_id: str) -> str | None:
        ...
        if self._task_is_hidden_by_delete_queue(task):
            ...
            if cleared:
                ...
                task_manager_module.logger.warning(
                    ...
                    "task_id=%s status=%s runtime_lease_owner=%s current_operation_id=%s",
                    task_id,
                    str(getattr(task, "status", "") or "").strip() or None,
                    runtime_lease_owner,
                    str(getattr(task, "current_operation_id", "") or "").strip() or None,
        ...
        if has_active_operation and current_status:
            ...
            if current_status != "pending" and operation_allows_runtime_resume:
                ...
                task_manager_module.logger.info(
                    ...
                    "task_id=%s status=%s runtime_phase=%s current_operation_id=%s operation_type=%s operation_status=%s runtime_lease_owner=%s",
                    task_id,
                    current_status,
                    self._task_runtime_phase(task),
                    current_operation_id,
        ...
        if current_status != "pending" and not operation_allows_runtime_resume and not has_active_operation:
            ...
            if nonpending_takeover_decision.allow_claim:
                ...
                task_manager_module.logger.info(
                    ...
                    "task_id=%s status=%s runtime_phase=%s previous_runtime_lease_owner=%s",
                    task_id,
                    current_status,
                    self._task_runtime_phase(task),
                    previous_runtime_lease_owner,
                )
                if current_status == "pending":
                    has_active_operation = self._operation_allows_owner_claim(current_operation)
                    operation_allows_runtime_resume = bool(
        ...
        if not lease_takeover_decision.allowed and (
            lease_takeover_decision.runtime_lease_owner
            or lease_takeover_decision.runtime_lease_expires_at is not None
        ):
        ...
        if (
            ...
            and final_claim_guard.decision_reason == "transition_guard_active"
            ...
            self._log_dispatch_claim_blocked(
                task_id,
                reason="claim_takeover_gate_suppressed_transition_guard",
                task=task,
                current_operation=current_operation,
            )
            self._set_dispatch_claim_decision(
                task_id=task_id,
                claimed_task_id=None,
                blocked_reason="claim_takeover_gate_suppressed_transition_guard",
                should_requeue=False,
            )
            return None
        started_at = task_manager_module._now()
    ...
    def _requeue_orphaned_owned_execution_locked(self: TaskManager, db: Session) -> bool:
        ...
        if isinstance(db, Session):
            session_factory = task_manager_module.get_session_factory()
            changed = False
            for task_id in self._owned_execution_requeue_candidate_task_ids(db):
                item_db = session_factory()
                try:
                    item_changed = self._requeue_orphaned_owned_execution_single_task_locked(item_db, task_id)
                    if item_changed:
                        item_db.commit()
                        changed = True
                    else:
                    ...
                    if self._is_retryable_lock_error(exc):
                        task_manager_module.logger.info(
                            "binary-security owned execution reclaim deferred by lock conflict: task_id=%s",
                            task_id,
                        )
                        continue
                    raise
    ...
    async def _run_task(self: TaskManager, task_id: str) -> None:
        ...
        try:
            ...
            ownership_snapshot = (
                self._parent_runtime_ownership_snapshot(db, task)
                if task is not None
                else None
            )
            control_operation_takeover = bool(
            ...
            if task is not None and (
                ...
                try:
                    ...
                    task_manager_module.logger.warning(
                        ...
                        "task_id=%s current_operation_id=%s runtime_lease_owner=%s",
                        task_id,
                        str(getattr(task, "current_operation_id", "") or "").strip() or None,
                        ownership_snapshot.runtime_lease_owner if ownership_snapshot is not None else None,
                    )
                    return
            if (
                task is None
                ...
                if task is not None:
                    ...
                    task_manager_module.logger.warning(
                        ...
                        "task_id=%s status=%s expected_instance_id=%s lease_active=%s current_operation_id=%s runtime_phase=%s runtime_lease_owner=%s",
                        task_id,
            ...
            task_manager_module.logger.info(
                ...
                self._task_runtime_phase(task),
                ownership_snapshot.runtime_lease_owner if ownership_snapshot is not None else None,
                ownership_snapshot.runtime_lease_expires_at if ownership_snapshot is not None else None,
            )
            ...
            execution_token = self._bind_execution_token(db, task)
            ...
            if task:
                failure_snapshot = self._parent_runtime_ownership_snapshot(db, task)
                if (
                    not active_commit_succeeded
                    and failure_snapshot.runtime_lease_owner == str(self.instance_id or "").strip()
                    ...
                    reopen_event_type, reopen_message = self._parent_runtime_reopen_allowed_event(
                        decision,
                        expired_message="父任务 authoritative runtime lease 已过期，允许 active commit 失败后重新排队",
                        missing_message="父任务 authoritative runtime lease 已缺失，允许 active commit 失败后重新排队",
                    )
                    self._record_parent_runtime_lease_decision(
                        db,
                        task,
                    ...
                    db.commit()
                    return
                current_token = self._dispatch_token_for_task(db, task)
                same_execution = (
                    failure_snapshot.runtime_lease_owner == str(self.instance_id or "").strip()
                ...
                if (
                    ...
                    if (
                        ...
                        self._requeue_owned_execution_takeover(
                            ...
                            event_payload={
                                "runtime_lease_owner": self._runtime_lease_log_view(cleanup_db, task)[0],
                                "current_operation_id": str(getattr(task, "current_operation_id", "") or "").strip() or None,
    ...
    async def _execute_task(self: TaskManager, task_id: str) -> None:
        ...
        try:
            ...
            if not task:
                return
            self._bind_execution_token(db, task)
    ...
    async def _run_stage_item_by_id(self: TaskManager, item_id: str) -> None:
        ...
        try:
            ...
            if task.status in task_manager_module.TASK_TERMINAL_STATUSES or task.status == "cancelled":
                return
            ownership_snapshot = self._parent_runtime_ownership_snapshot(db, task)
            current_task_owner = str(ownership_snapshot.runtime_lease_owner or "").strip()
            if not (ownership_snapshot.runtime_lease_active and current_task_owner == str(self.instance_id or "").strip()):
    ...
    def _dispatch_token(self: TaskManager, task) -> str | None:
        cached_token = str(getattr(task, "_execution_token", "") or "").strip() or None
        if cached_token:
            return cached_token
        runtime_owner = str(getattr(task, "_execution_dispatcher_id", "") or "").strip() or None
        runtime_started_at = getattr(task, "_runtime_lease_owner_started_at", None)
        runtime_generation = getattr(task, "_runtime_lease_generation", None)
        runtime_epoch = getattr(task, "_runtime_lease_execution_epoch", None)
        runtime_token = self._compose_runtime_lease_execution_token(
            owner_instance_id=runtime_owner,
            owner_started_at=runtime_started_at,
            generation=runtime_generation,
            execution_epoch=runtime_epoch,
        )
