...
class TaskManager:
    ...
    async def stop(self) -> None:
        ...
        if self._archive_loop_task:
            ...
            try:
                ...
                pass
        if getattr(self, "_operation_loop_task", None):
            self._operation_loop_task.cancel()
            try:
                await self._operation_loop_task
            except asyncio.CancelledError:
                pass
    ...
    def _task_policy_update_support(self, task: BinarySecurityTask, db: Session | None = None) -> tuple[bool, str | None]:
    ...
    async def cancel_task(self, db: Session, *, project_id: str, task_id: str) -> BinarySecurityActionResponse:
        ...
        if active_cancel_operation is not None and str(active_cancel_operation.operation_type or "").strip() == TASK_ACTION_CANCEL:
            observe_task_operation("cancel", "already_queued")
            return BinarySecurityActionResponse(
                task_id=task_id,
    ...
    def _retry_target_items(
        ...
        *,
        ...
        retry_item_keys = {
            str(item_key).strip()
            for item_key in list(plan.get("retry_item_keys") or [])
    ...
    def _enqueue_operation(self, operation_id: str) -> None:
        if not self.cfg.queue.enabled:
            return
        try:
            loop = asyncio.get_running_loop()
        ...
        try:
            loop.create_task(
                self._run_scheduled_coroutine(
                    get_task_queue().push_operation(operation_id),
    ...
    def _claim_operation_by_id(self, db: Session, operation_id: str) -> BinarySecurityTaskOperation | None:
        now_value = _now()
        lease_expires_at = self._operation_lease_expires_at(now_value=now_value)
        ...
        updated = (
            db.query(BinarySecurityTaskOperation)
            .filter(
                BinarySecurityTaskOperation.id == operation_id,
                BinarySecurityTaskOperation.status.in_(["accepted", "queued", "claimed", "running"]),
                or_(
                    BinarySecurityTaskOperation.owner_instance_id.is_(None),
                    BinarySecurityTaskOperation.owner_instance_id == self.instance_id,
                    BinarySecurityTaskOperation.claim_lease_expires_at.is_(None),
                    BinarySecurityTaskOperation.claim_lease_expires_at < now_value,
                ),
            )
            .update(
                {
                    BinarySecurityTaskOperation.status: "running",
                    BinarySecurityTaskOperation.owner_instance_id: self.instance_id,
                    BinarySecurityTaskOperation.started_at: operation.started_at or now_value,
                    BinarySecurityTaskOperation.heartbeat_at: now_value,
                    BinarySecurityTaskOperation.claim_lease_expires_at: lease_expires_at,
                    BinarySecurityTaskOperation.updated_at: now_value,
                },
        ...
        if not updated:
            return None
        db.commit()
        return db.query(BinarySecurityTaskOperation).filter(BinarySecurityTaskOperation.id == operation_id).first()

    def _renew_operation_lease(self, db: Session, operation_id: str) -> bool:
        now_value = _now()
        lease_expires_at = self._operation_lease_expires_at(now_value=now_value)
        updated = (
            db.query(BinarySecurityTaskOperation)
    ...
    async def _operation_lease_heartbeat(self, operation_id: str) -> None:
        ...
        while self._running:
            db = get_session_factory()()
            try:
                renewed = self._renew_operation_lease(db, operation_id)
                if not renewed:
                    operation = db.query(BinarySecurityTaskOperation).filter(BinarySecurityTaskOperation.id == operation_id).first()
                    observe_control_operation_lease_lost(operation.operation_type if operation is not None else "unknown")
                    return
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("binary-security operation lease heartbeat failed: %s", operation_id)
            finally:
                db.close()
            await asyncio.sleep(interval_seconds)

    def _requeue_stale_operations(self, db: Session) -> bool:
        now_value = _now()
        stale = (
            db.query(BinarySecurityTaskOperation)
            .filter(
                BinarySecurityTaskOperation.status == "running",
                BinarySecurityTaskOperation.claim_lease_expires_at.isnot(None),
                BinarySecurityTaskOperation.claim_lease_expires_at < now_value,
            )
            .all()
        )
        ...
        for operation in stale:
            if str(operation.owner_instance_id or "").strip() == self.instance_id and operation.id in self._operation_workers:
                continue
            ...
            operation.error_code = "operation_lease_lost"
            operation.error_message = "操作执行租约已失效，等待重新接管"
            observe_control_operation_lease_lost(operation.operation_type)
            task = db.query(BinarySecurityTask).filter(BinarySecurityTask.id == operation.task_id).first()
            if task is not None:
                task.current_operation_id = operation.id
                self._record_operation_event(
                    ...
                    task,
                    operation,
                    "operation_requeued_after_lease_lost",
                    f"后台操作租约已失效，等待重新接管: {operation.operation_type}",
                    level="warning",
    ...
    def _latest_operation(self, db: Session, task_id: str) -> BinarySecurityTaskOperation | None:
        return (
            db.query(BinarySecurityTaskOperation)
            .filter(BinarySecurityTaskOperation.task_id == task_id)
            .order_by(BinarySecurityTaskOperation.created_at.desc(), BinarySecurityTaskOperation.id.desc())
            .first()
        )
    ...
    def _list_task_operations(self, db: Session, task_id: str) -> list[BinarySecurityTaskOperation]:
        return (
            db.query(BinarySecurityTaskOperation)
            .filter(BinarySecurityTaskOperation.task_id == task_id)
            .order_by(BinarySecurityTaskOperation.created_at.desc(), BinarySecurityTaskOperation.id.desc())
    ...
    def _operation_resume_step(self, operation: BinarySecurityTaskOperation) -> str:
        cursor = dict(operation.resume_cursor or {})
        current_step = str(cursor.get("current_step") or operation.current_step or "").strip()
        valid_steps = TASK_CANCEL_SAGA_STEPS if str(operation.operation_type or "").strip() == TASK_ACTION_CANCEL else TASK_OPERATION_SAGA_STEPS
        if current_step in valid_steps:
            return current_step
        if str(operation.operation_type or "").strip() == TASK_ACTION_CANCEL:
            return TASK_OPERATION_STEP_MARK_TASK_CANCELLING
        return TASK_OPERATION_STEP_COLLECT_CLEANUP_PLAN

    def _operation_step_snapshot(self, operation: BinarySecurityTaskOperation, step_name: str) -> dict[str, Any]:
        return dict(dict(operation.step_payload or {}).get(step_name) or {})

    def _cancel_verify_timeout_seconds(self) -> int:
        scheduler_cfg = getattr(self.cfg, "scheduler", None)
        request_timeout = int(getattr(scheduler_cfg, "downstream_request_timeout_seconds", 120) or 120)
        poll_interval = int(getattr(scheduler_cfg, "stage_poll_interval_seconds", 5) or 5)
        return max(TASK_CANCEL_VERIFY_TIMEOUT_SECONDS, request_timeout, poll_interval * 6)

    def _latest_cancel_operation(self, db: Session, task_id: str) -> BinarySecurityTaskOperation | None:
        return (
            db.query(BinarySecurityTaskOperation)
            .filter(
                BinarySecurityTaskOperation.task_id == task_id,
                BinarySecurityTaskOperation.operation_type == TASK_ACTION_CANCEL,
            )
            .order_by(BinarySecurityTaskOperation.created_at.desc(), BinarySecurityTaskOperation.id.desc())
            .first()
        )
    ...
    def _cancel_target_key(self, target: dict[str, Any]) -> str:
        target_type = str(target.get("target_type") or "").strip()
        if target_type == "local_worker":
            return "local_worker"
        if target_type == "stage_item":
            return f"stage_item:{target.get('item_id')}"
        return f"downstream:{target.get('downstream_service')}:{target.get('downstream_task_id')}"

    def _cancel_target_is_terminal(self, observation_status: str) -> bool:
        return str(observation_status or "").strip() in {"cancelled", "success", "failed", "missing"}

    def _cancel_target_display(self, target: dict[str, Any]) -> str:
        target_type = str(target.get("target_type") or "").strip()
        if target_type == "local_worker":
            return f"local_worker:{target.get('task_id') or '-'}"
        if target_type == "stage_item":
            return f"stage_item:{target.get('stage_name') or '-'}:{target.get('item_id') or '-'}"
        return f"{target.get('downstream_service') or '-'}:{target.get('downstream_task_id') or '-'}"

    def _normalize_cancel_targets(self, targets: list[dict[str, Any]]) -> list[dict[str, Any]]:
        deduped: dict[str, dict[str, Any]] = {}
        for raw_target in targets:
            if not isinstance(raw_target, dict):
                continue
            target = dict(raw_target)
            target.setdefault("cancel_request_status", "not_sent")
            target.setdefault("terminal_observation_status", "unknown")
            target.setdefault("blocking", True)
            key = self._cancel_target_key(target)
            current = deduped.get(key)
            if current is None:
                deduped[key] = target
                continue
            merged = {**current, **target}
            merged["blocking"] = bool(current.get("blocking")) or bool(target.get("blocking"))
            deduped[key] = merged
    ...
    def _collect_cancel_targets(
        ...
        if str(task.status or "").strip() not in TASK_TERMINAL_STATUSES or str(task.current_operation_id or "").strip():
            targets.append(
                {
            ...
            targets.append(
                {
                    ...
                    "blocking": True,
                    "cancel_request_status": "requested",
                    "terminal_observation_status": self._normalize_item_status(item.status),
                    "last_observed_at": _isoformat_or_none(_now()),
                    "error": None,
                }
            )
            ...
            targets.append(
                {
                    ...
                    "item_key": ref.get("item_key"),
                    "downstream_service": ref.get("service"),
                    "downstream_task_id": ref.get("task_id"),
                    "project_id": ref.get("project_id"),
                    "blocking": True,
                    "cancel_request_status": "not_sent",
                    "terminal_observation_status": "unknown",
                    "last_observed_at": None,
                    "error": None,
                }
    ...
    def _create_task_operation(
        ...
        *,
        ...
        observe_control_operation(operation_type, "accepted")
        return operation
    ...
    def _queue_task_operation(
        ...
        *,
        ...
        db.commit()
        self._enqueue_operation(operation.id)
        observe_control_operation(operation.operation_type, "queued")
        return operation
    ...
    async def _reconcile_work_queues(self, db: Session, *, force: bool = False) -> None:
        ...
        operation_ids = [
            ...
            .filter(BinarySecurityTaskOperation.status.in_(["accepted", "queued"]))
            .order_by(BinarySecurityTaskOperation.updated_at.asc(), BinarySecurityTaskOperation.created_at.asc(), BinarySecurityTaskOperation.id.asc())
            .limit(seed_batch)
            .all()
        ]
    ...
    async def _prepare_archive_retry_full(
        ...
        supported, reason, jobs, stage_items = self._archive_full_retry_support(db, task, target_stage, ignore_operation_lock=True)
        if not supported:
    ...
    async def _prepare_cancel_task(self, db: Session, task: BinarySecurityTask) -> list[str]:
        token = self._service_token()
        ...
        if downstream_refs:
            await self._cancel_downstream_refs(db, task, downstream_refs, token)
        return sorted({str(stage_run.stage_name or "").strip() for stage_run in active_stage_runs if str(stage_run.stage_name or "").strip()})

    def _cancel_target_observation_status(self, payload: dict[str, Any] | None) -> str:
        raw_payload = dict(payload or {})
        mapped_status = self._map_downstream_status(str(raw_payload.get("status") or "")) or ""
        ...
        if mapped_status == "failed":
            return "failed"
        if bool(raw_payload.get("cancel_requested")):
            return "cancelling"
    ...
    async def _run_cancel_operation_steps(
        ...
        cancel_steps = TASK_CANCEL_SAGA_STEPS
        current_step = resume_step if resume_step in cancel_steps else TASK_OPERATION_STEP_MARK_TASK_CANCELLING
        ...
        async def _verify_quiesced() -> dict[str, Any]:
            ...
            while True:
                ...
                task_items = {
                    item.id: item
                    for item in db.query(BinarySecurityStageItem).filter(BinarySecurityStageItem.task_id == task.id).all()
                }
                ...
                db.expire_all()
                task_refreshed = db.query(BinarySecurityTask).filter(BinarySecurityTask.id == task.id).first()
                operation_refreshed = db.query(BinarySecurityTaskOperation).filter(BinarySecurityTaskOperation.id == operation.id).first()
        ...
        async def _finalize_cancel_failed(error: Exception) -> dict[str, Any]:
            ...
            await self._write_task_metadata_async(task, Path(task.workspace_root) / "input" / "task-metadata.json", status=TASK_STATUS_CANCEL_FAILED)
        ...
        try:
            await _run_step(
                ...
                next_step=TASK_OPERATION_STEP_COLLECT_CANCEL_TARGETS,
                fn=_mark_task_cancelling,
            )
            await _run_step(
                TASK_OPERATION_STEP_COLLECT_CANCEL_TARGETS,
                message="取消操作已收集需要收敛的目标",
                next_step=TASK_OPERATION_STEP_CANCEL_LOCAL_EXECUTION,
                fn=_collect_targets,
            )
            await _run_step(
                TASK_OPERATION_STEP_CANCEL_LOCAL_EXECUTION,
                message="取消操作已停止本地执行链",
                next_step=TASK_OPERATION_STEP_CANCEL_DOWNSTREAM_TARGETS,
                fn=_cancel_local,
            )
            await _run_step(
                TASK_OPERATION_STEP_CANCEL_DOWNSTREAM_TARGETS,
                message="取消操作已向下游发出取消请求",
                next_step=TASK_OPERATION_STEP_VERIFY_DOWNSTREAM_QUIESCED,
                fn=_cancel_downstream_targets,
            )
            await _run_step(
                TASK_OPERATION_STEP_VERIFY_DOWNSTREAM_QUIESCED,
                message="取消操作已完成下游收敛核验",
                next_step=TASK_OPERATION_STEP_FINALIZE_TASK_CANCELLED,
                fn=_verify_quiesced,
            )
        except Exception as exc:
            await _run_step(
                TASK_OPERATION_STEP_FINALIZE_TASK_CANCEL_FAILED,
                message="取消操作已收口为失败",
                next_step=TASK_OPERATION_STEP_FINALIZE_TASK_CANCEL_FAILED,
    ...
    async def _operation_dispatch_loop(self) -> None:
        session_factory = get_session_factory()
        while self._running:
            operation_id = None
            db = session_factory()
    ...
    async def _run_task_operation(self, operation_id: str) -> None:
        ...
        try:
            ...
            if operation.operation_type == TASK_ACTION_DELETE:
                task_deleted = db.query(BinarySecurityTask.id).filter(BinarySecurityTask.id == operation.task_id).first() is None
            operation.heartbeat_at = _now()
            operation.claim_lease_expires_at = _now()
            ...
            db.commit()
            observe_control_operation(operation_type, "succeeded")
        except Exception as exc:
            db.rollback()
            operation = db.query(BinarySecurityTaskOperation).filter(BinarySecurityTaskOperation.id == operation_id).first()
    ...
    async def _run_task_operation_steps(
        self,
        db: Session,
        task: BinarySecurityTask,
        operation: BinarySecurityTaskOperation,
        ...
        if operation.operation_type == TASK_ACTION_CANCEL:
            await self._run_cancel_operation_steps(db, task, operation, resume_step)
            return
        if operation.operation_type in {TASK_ACTION_RETRY_FAILED_ITEMS, TASK_ACTION_RETRY_STAGE_FAILED_ITEMS}:
        ...
        if self._is_operation_requeue_state_applied(task, operation):
            self._record_operation_step_finished(
                ...
                task,
                operation,
                step_name=TASK_OPERATION_STEP_REQUEUE_TASK,
        ...
        task.status = "pending"
        task.current_stage = operation.target_stage or task.current_stage
        task.last_error = None
        task.finished_at = None
        self._invalidate_task_execution(task)
        self._enqueue_task(task.id)
        ...
        self._record_operation_event(
            ...
            operation,
            "task_requeued",
            f"后台操作完成，任务已重新排队: {operation.operation_type}",
            stage_name=operation.target_stage,
        )
        self._record_operation_step_finished(
            db,
    ...
    async def _run_retry_failed_items_operation_steps(
        self,
        db: Session,
        task: BinarySecurityTask,
        operation: BinarySecurityTaskOperation,
        ...
        current_step = resume_step if resume_step in {row[0] for row in step_flow} else TASK_OPERATION_STEP_COLLECT_CLEANUP_PLAN
        ...
        if current_step != TASK_OPERATION_STEP_REQUEUE_TASK:
            return
        if self._is_operation_requeue_state_applied(task, operation):
            self._record_operation_step_finished(
                db,
    ...
    async def _operation_cleanup_retry_abnormal_children(
        ...
        for action in self._retry_item_actions(task):
            if str(action.get("strategy") or "").strip() != RETRY_CHILD_STRATEGY_RECREATE_FROM_ABNORMAL:
                continue
            item = db.query(BinarySecurityStageItem).filter(BinarySecurityStageItem.id == str(action.get("item_id") or "")).first()
            if item is None:
                raise ValidationError(f"失败项重试目标缺失: {action.get('item_id')}")
            old_task_id = str(action.get("old_downstream_task_id") or "").strip()
            self._record_operation_event(
                db,
                task,
                operation,
                "retry_item_cleanup_started",
                f"开始清理旧下游子任务: {item.downstream_service}:{old_task_id or '-'}",
                stage_name=item.stage_name,
            ...
            self._record_operation_event(
                ...
                task,
                operation,
                "retry_item_cleanup_succeeded",
                f"旧下游子任务已清理: {item.downstream_service}:{old_task_id or '-'}",
                stage_name=item.stage_name,
                payload={"item_id": item.id, "item_key": item.item_key, "old_downstream_task_id": old_task_id},
            )
            cleaned += 1
        ...
        for action in self._retry_item_actions(task):
            ...
            if str(item.downstream_task_id or "").strip():
                self._update_retry_item_action(
                    ...
                    item_id=item.id,
                    updates={
                        "new_downstream_task_id": str(item.downstream_task_id or "").strip(),
            ...
            self._update_retry_item_action(
                ...
                item_id=item.id,
                updates={
                    "current_downstream_task_id": new_task_id,
            ...
            self._record_operation_event(
                ...
                f"替换子任务已创建: {service}:{new_task_id}",
                stage_name=item.stage_name,
        ...
        for action in self._retry_item_actions(task):
            ...
            self._record_operation_event(
                ...
                payload={
                    ...
                    "item_key": item.item_key,
                    "strategy": strategy,
                    "old_downstream_task_id": action.get("old_downstream_task_id"),
    ...
    def _task_sync_cooldown_elapsed(self, db: Session, task: BinarySecurityTask) -> bool:
        return self._task_sync_cooldown_elapsed_for_items(
            task,
            self._task_reconcile_candidate_items(db, task),
        )
    ...
    def _task_sync_cooldown_elapsed_for_items(
        self,
        task: BinarySecurityTask,
    ...
    async def _observe_runtime_metrics(self, db: Session, *, reconcile_candidates: int | None = None) -> None:
        ...
        observe_queue_depths(
            ...
            redis_operation_queue=int((queue_snapshot.get("operation_queue") or {}).get("length", 0)),
            ...
            operation_queue_oldest_age_seconds=float((queue_snapshot.get("operation_queue") or {}).get("oldest_age_seconds", 0.0) or 0.0),
        )
        observe_slot_usage(
        ...
        *,
        ...
        if active_operation is not None:
            ...
            return {
                ...
                "operation_started_at": _isoformat_or_none(active_operation.started_at),
                "operation_expires_at": _isoformat_or_none(active_operation.claim_lease_expires_at),
                "operation_heartbeat_at": _isoformat_or_none(active_operation.heartbeat_at),
                "can_cancel": False,
                "can_continue": False,
                "can_retry": False,
    ...
    def _build_manual_operation_state(
        ...
        *,
        ...
        can_cancel = task.status not in TASK_TERMINAL_STATUSES
    ...
    def _retry_recreate_payload_for_item(
        ...
        if service == "dataflow_analyse":
            entry = self._validate_entry_output_contract(input_ref, allow_fallback=True)
            taint_params = [str(value).strip() for value in (entry.get("taint_params") or []) if str(value).strip()] or _entry_signature_params(entry)
            definition_kind = self._resolve_entry_definition_kind(entry)
            definition_line = str(entry.get("definition_line") or entry.get("line_no") or "").strip()
            normalized_source_file = self._normalize_dfa_source_file(str(entry.get("source_root_path") or ""), entry)
            prompt = f"分析文件 {str(entry.get('definition_file') or entry['file_name'])} 中函数 {entry['function_name']} 的外部输入数据流"
            ...
            return service, None, {
                ...
                "module_input_path": entry["module_input_path"],
        ...
        if service == "dataflow_vuln_scanner":
            dataflow_result = self._validate_dataflow_output_contract(input_ref, allow_fallback=True)
            dataflow_input_dir = self._resolve_vuln_scan_dataflow_input_dir(dataflow_result)
            ...
            if not dataflow_input_dir or not source_dir:
                raise ValidationError("vuln_scan retry 缺少 dataflow/source 输入")
            return service, token, {
                "title": f"{task.name}-{dataflow_result['function_name']}-scan",
                "data_flow_path": dataflow_input_dir,
                "source_dir": source_dir,
