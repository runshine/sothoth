...
class TaskStageRuntimeTests(unittest.TestCase):
    ...
    def test_refresh_kg_streaming_inputs_if_needed_does_not_duplicate_when_module_key_spelling_changes(self):
        task = BinarySecurityTask(
            id="task-kg-reseed-dedupe",
            project_id="project-1",
            name="task",
            workspace_root="/tmp/ws",
            output_root="/tmp/out",
        ...
        authoritative_item = BinarySecurityStageItem(
            ...
            item_identity_key="entry-a::knowledge-graph-source-project",
            status="running",
            downstream_service="dataflow_vuln_scan",
            downstream_task_id="child-existing",
        )
        db = _ModelAwareDb(tasks=[task], stage_runs=[kg_run, dataflow_run], stage_items=[authoritative_item], events=[])

        before_entry = {"entry_key": "entry-a", "function_name": "fn_a", "module_key": "knowledge-graph-source-project"}
        after_entry = {"entry_key": "entry-a", "function_name": "fn_a", "module_key": "knowledge-graph-source-project"}
        effective_inputs = [[before_entry], [after_entry], [after_entry]]
        ...
        with (
            ...
            patch.object(self.manager, "_latest_stage_run", side_effect=lambda _db, _task_id, stage: kg_run if stage == "knowledge_graph_entry_fetch" else dataflow_run if stage == "dataflow_vuln_scan" else None),
            patch.object(self.manager, "_effective_entry_inputs", side_effect=lambda *_args, **_kwargs: effective_inputs.pop(0)),
            patch.object(self.manager, "_stage_items", return_value=[authoritative_item]),
            patch.object(self.manager, "_stage_parallelism", return_value=5),
            patch.object(self.manager, "_queued_stage_child_create_item_ids", return_value=set()),
            patch.object(self.manager, "_run_async_blocking", side_effect=lambda value: value),
            patch.object(self.manager, "_stage_knowledge_graph_entry_fetch", new=Mock(return_value=True)),
        ...
        self.assertTrue(changed)
        prepare_items.assert_not_called()
