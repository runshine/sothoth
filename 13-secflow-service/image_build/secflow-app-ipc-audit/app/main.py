...
async def lifespan(_: FastAPI):
    logger.info("starting secflow ipc audit service...")
    load_config()
    cfg = get_config()
    ...
    logger.info("secflow ipc audit service started")
    yield
    await get_scheduler_service().stop()
    logger.info("secflow ipc audit service stopped")


def create_app() -> FastAPI:
    app = FastAPI(
        title="SecFlow IPC Audit Service",
        description="独立的 OpenHarmony IPC 审计与 PoC 执行微服务",
        version="0.1.0",
        lifespan=lifespan,
    )
    app.add_middleware(
