...
class CodexRuntime(BaseAgentRuntime):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info("codex_cli_available", version=stdout.decode().strip(),
                        agent_id=self.agent_id)
    ...
    async def send_message(
        ...
        # Codex 使用 --quiet 和 prompt 参数
        full_prompt = message
        if system_prompt:
            full_prompt = f"{system_prompt}\n\n{message}"

        cmd.extend(["--quiet", "--prompt", full_prompt])

        if self.runtime_config.get("sdk_specific", {}).get("sandbox"):
            cmd.append("--sandbox")
        ...
        try:
            ...
            return AgentResponse(content="", error=f"Codex 超时 ({timeout}s)")
        except FileNotFoundError:
            return AgentResponse(content="", error="codex CLI 未安装")
