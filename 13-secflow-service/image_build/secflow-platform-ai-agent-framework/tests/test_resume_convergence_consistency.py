...
def test_rebuild_review_state_restores_blockers_closure_and_passed_results(tmp_path: Path) -> None:
    ...
    assert state.get_open_blocker_ids() == ["export-followthrough:send-socket"]
...
@pytest.mark.asyncio
async def test_resume_run_preserves_blockers_closure_and_passed_results(
    ...
    assert rebuilt_state.get_open_blocker_ids() == ["export-followthrough:send-socket"]
    ...
    assert any("result_001.md" in msg and "已通过评审的结果" in msg for msg in runtime_state["worker_rework_messages"])
