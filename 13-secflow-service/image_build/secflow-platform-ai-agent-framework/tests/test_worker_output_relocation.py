...
def test_rework_prompt_filters_framework_blockers_and_stale_deleted_results(tmp_path: Path):
    work_dir = tmp_path / "work"
    ...
    state.record_global_review_result(
        ...
        blocking_issues=[
            ...
            },
            {
                ...
                "severity": "high",
                "required_action": "同步 open_blockers.json 与 resolved_issues",
    ...
    assert prompt.count("当前未关闭的 Worker 可执行 blocker backlog") == 1
    assert "尚未关闭的全局阻塞项（必须逐项处理，不得静默删除）" not in prompt
...
def test_reflection_scope_becomes_mode_aware_in_rework_cycles(tmp_path: Path):
    ...
    assert "worker-gap" in scope
    assert "supporting_docs" in scope


def test_rework_prompt_switches_to_result_repair_only_without_worker_blockers(tmp_path: Path):
    work_dir = tmp_path / "work"
    work_dir.mkdir(parents=True, exist_ok=True)
    (work_dir / "summary.md").write_text("## 7. 局限性与未覆盖区域\n\n- old gap\n", encoding="utf-8")
    ...
    assert "当前没有需要 Worker 继续扩展的 blocker backlog" in prompt
    assert "只聚焦**修复/删除未通过结果**" in prompt
    assert "不要继续扩张攻击面" in prompt


def test_rework_prompt_marks_blockers_on_protected_results_as_no_direct_edit(tmp_path: Path):
    work_dir = tmp_path / "work"
    work_dir.mkdir(parents=True, exist_ok=True)
    (work_dir / "summary.md").write_text("## 7. 局限性与未覆盖区域\n\n- old gap\n", encoding="utf-8")
    ...
    assert "命中已冻结结果的 blocker" in prompt
    assert "禁止直接改原文件" in prompt
    assert "新增更高编号的补充报告" in prompt
