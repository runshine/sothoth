...
| 9 | StkGlbPolicy → 协议解析 | USED | ✅ | 边界检查存在逻辑漏洞 |
| 10 | MemoryContinuous_fl → 内存访问 | USED | ✅ | 长度验证不完整 |
...
| 004 | result_004.md | LDM_RcvEventIPv6SrhTtlExceed | CWE-119 缓冲区溢出 | High | 高 | 内存操作缺乏长度验证可能导致溢出 |
| 005 | result_005.md | LDM_VerifyIpv6SliceHdrAndUpdateIfIdx | CWE-754 校验不完整 | Low-Medium | 中 | 策略值边界检查存在逻辑漏洞 |
...
| LDM_RcvEventIPv6SrhTtlExceed | 1 | 0 | 0 |
| LDM_VerifyIpv6SliceHdrAndUpdateIfIdx | 0 | 0 | 1 |

### 建议修复优先级

1. **P0 (紧急)**: 修复偏移量字段边界校验漏洞（漏洞001、006）
2. **P1 (高)**: 修复内存操作长度验证问题（漏洞004）
3. **P2 (中)**: 增强 ProtoType 范围校验（漏洞002）
4. **P3 (低)**: 完善策略值边界检查逻辑（漏洞005）
