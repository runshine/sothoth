...
def _is_runtime_temp_dir(path: str | Path | None) -> bool:
    if not path:
        return False
    try:
...
def _is_runtime_temp_path(path: str | Path | None) -> bool:
    if not path:
        return False
    try:
...
def _remove_ida_intermediate_outputs(output_dir: str | Path | None) -> list[str]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_dir():
            if _is_runtime_temp_dir(path):
                temp_dirs.append(path)
            continue
        if path.is_file() and _is_ida_intermediate_path(path) and not _is_runtime_temp_path(path.relative_to(root)):
