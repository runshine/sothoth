...
def _find_atomic_work_dir(run_dir: Path) -> Path | None:
    ...
    workspace_root = ((cfg.get("global") or {}).get("workspace_root") or "")
    if workspace_root:
        workspace_candidates.append(Path(from_msys_path(workspace_root) or workspace_root))
    workspace_candidates.extend([run_dir / "workspace", run_dir / "ws"])
...
def get_run_detail(name: str):
    ...
    if not atomic:
        ...
        return {"config": cfg_summary, "status": status, "start_time": start_time, "start_epoch": int(_find_run_start_epoch(run_dir, run_meta) or 0), "start_date": start_date, "duration_seconds": _compute_duration(run_dir, None, status, run_meta), "last_activity": "", "cycles": [], "results": [], "latest_issues": []}
