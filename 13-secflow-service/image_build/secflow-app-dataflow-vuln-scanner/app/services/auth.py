...
class AuthService:
    ...
    async def startup_validate(self) -> None:
        ...
        if not machine_token:
            raise RuntimeError("auth service machine token is required when auth_service.enabled=true")
        async with httpx.AsyncClient(timeout=config.timeout) as client:
            response = await client.post(
                config.machine_validate_url,
                headers={"Authorization": f"Bearer {machine_token}", **self._service_headers()},
            )
        if response.status_code != 200:
            raise RuntimeError(f"auth service validation failed: {response.status_code} {response.text}")
