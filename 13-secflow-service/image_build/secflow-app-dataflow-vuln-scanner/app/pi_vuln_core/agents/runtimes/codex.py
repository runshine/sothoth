...
logger = get_logger("runtime.codex")


class CodexRuntime(BaseAgentRuntime):
    """OpenAI Codex CLI 运行时"""

    async def initialize(self) -> None:
        api_key_env = self.runtime_config.get("api_key_env", "OPENAI_API_KEY")
        ...
        try:
            ...
            if proc.returncode == 0:
                logger.info("codex_cli_available", version=stdout.decode().strip(), agent_id=self.agent_id)
            else:
                logger.warning(
                    "codex_cli_not_found",
            ...
            logger.warning("codex_cli_not_found", agent_id=self.agent_id)
        self._initialized = True

    async def create_session(self) -> str:
        session_id = f"codex_{uuid.uuid4().hex[:12]}"
    ...
    async def send_message(
        ...
        model = self.runtime_config.get("model", "o3-mini")
        sandbox = bool(sdk_cfg.get("sandbox"))
        ...
        cmd = ["codex", "--model", model, "--quiet", "--prompt", full_prompt]
        if sandbox:
            cmd.append("--sandbox")
        ...
        try:
            ...
            if proc.returncode != 0 and not output:
                error = f"codex exit code={proc.returncode}: {stderr_text[:500]}"
            ...
            return AgentResponse(content="", error=f"Codex 超时 ({timeout}s)", conversation_id=session_id)
        except FileNotFoundError:
            trace_context.write_result(
                stdout_text="",
                stderr_text="",
            ...
            return AgentResponse(content="", error="codex CLI 未安装", conversation_id=session_id)
        finally:
