...
class BaseAgentRuntime(ABC):
    ...
    async def create_session_with_hint(self, session_hint: Optional[str] = None) -> str:
        ...
        默认实现回退到 create_session()，这样旧 runtime / 测试 double 无需同步改签名；
        真实 runtime 可覆写以生成更可读的 session 目录名。
        """
    ...
    def _sanitize_session_hint(self, session_hint: str) -> str:
        text = re.sub(r"[^A-Za-z0-9._-]+", "_", (session_hint or "").strip())
        text = re.sub(r"_+", "_", text).strip("._-")
        return text[:96] or f"session_{uuid.uuid4().hex[:8]}"

    def _reserve_session_id(self, preferred_id: str) -> str:
        candidate = self._sanitize_session_hint(preferred_id)
        if candidate not in self._sessions:
            return candidate

        index = 2
        while True:
            scoped = f"{candidate}_{index}"
            if scoped not in self._sessions:
                return scoped
            index += 1
