        ...
        **build_service_meta(),
        ...
        "checks": {
            "bootstrap": {
                ...
                "attempts": int(runtime_status.get("attempts") or 0),
            },
            "auth": {"ok": bool(state.get("auth_ready")), "detail": {"cached": True}},
            "project": {"ok": bool(state.get("project_ready")), "detail": {"cached": True}},
            "registry": {"ok": bool(state.get("registry_ready") or scheduler.role in {"worker", "manager"}), "detail": {"cached": True}},
            "scheduler": {
                "ok": bool(scheduler_payload.get("scheduler") == "running" or scheduler.role == "api"),
                "role": scheduler.role,
                "worker_enabled": bool(scheduler.runs_worker),
                "status": scheduler_payload.get("scheduler"),
            },
        },
    }
