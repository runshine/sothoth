...
from app.api.dependencies import get_current_subject, get_db
...
def list_workers(subject=Depends(get_current_subject), db: Session = Depends(get_db)):
    return get_scheduler_service().list_workers(db)


@router.get("/scheduler/workers/{pod_id}", response_model=SchedulerWorkerResponse)
def get_worker(pod_id: str, subject=Depends(get_current_subject), db: Session = Depends(get_db)):
...
def drain_worker(pod_id: str, subject=Depends(get_current_subject), db: Session = Depends(get_db)):
...
def activate_worker(pod_id: str, subject=Depends(get_current_subject), db: Session = Depends(get_db)):
