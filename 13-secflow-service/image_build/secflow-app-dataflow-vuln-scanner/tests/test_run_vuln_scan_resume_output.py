...
def test_collect_resume_diagnostics_reads_cycle_files_and_review_state(tmp_path: Path) -> None:
    ...
    (meta_dir / "review_feedback").mkdir(parents=True, exist_ok=True)
    ...
    (meta_dir / "review_feedback" / "cycle_003.json").write_text(
        json.dumps(
            {
                "cycle": 3,
                "issues": [
    ...
    state = ReviewState()
    state.activate_closure_mode(3, "issue IDs 未变化")
    ...
    assert diagnostics["plateau_reason"] == "issue IDs 未变化"
    assert diagnostics["scores"]["export_followthrough"] == 0.5
    assert diagnostics["issues_preview"][0].startswith("[export-followthrough:send-socket]")


def test_format_resume_diagnostic_lines_includes_mode_plateau_and_issues() -> None:
    diagnostics = {
        "latest_cycle": 3,
        ...
        "plateau_reason": "issue IDs 未变化；summary/result 未表现出收缩趋势",
        "issues_preview": [
    ...
    assert "Plateau原因: issue IDs 未变化；summary/result 未表现出收缩趋势" in rendered
    assert "主要Issue:" in rendered
...
def test_write_resume_preview_file_persists_diagnostics(tmp_path: Path) -> None:
    ...
    assert payload["diagnostics"]["issue_count"] == 2
