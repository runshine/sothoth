...
@pytest.mark.asyncio
async def test_atomic_state_tracks_previous_state(framework_config_payload: dict, tmp_path: Path, patch_mock_agent_runtime):
    input_task = tmp_path / "input-task.md"
    input_task.write_text("# Test Task\n\nAnalyze mock binary.\n", encoding="utf-8")
    ...
    state_files = list(workflow_dir.rglob("_meta/state.json"))
    assert len(state_files) == 1

    state_payload = json.loads(state_files[0].read_text(encoding="utf-8"))
    assert state_payload["current_state"] == "completed"
    assert state_payload["previous_state"] == "end_plugins"
