...
def test_run_vuln_scan_resume_preview_and_actual_resume_follow_design_logic(
    ...
    assert state["global_review_calls"] == 6  # 2 advisors × 3 cycles (parallel)
    assert state["result_review_calls"] == ["result_001.md"]
    assert all("总结报告:" in msg or "summary file:" in msg.lower() for msg in state["global_review_messages"])
    ...
    assert exit_code == 0
    assert "当前模式:" in captured.out
    assert "Plateau:" in captured.out
    assert "dry-run-resume" in captured.out
    assert state["global_review_calls"] == 6  # dry-run 不会调用任何 agent
    ...
    assert preview["diagnostics"]["workflow_mode"] in ("discovery", "closure")
    # Issue-based diagnostics removed
    # assert preview["diagnostics"]["issue_count"] == 1
    # assert preview["diagnostics"]["issues_preview"][0].startswith("[global_completeness:export-followthrough:send-socket]")
    ...
    assert state["global_review_calls"] == 8  # 6 + 2 advisors × 1 resume cycle
    # Closure mode and issue assertions removed for lightweight feedback chain
    # assert any("当前已经进入 **closure（收敛）模式**" in msg for msg in state["worker_rework_messages"])
    assert any("result_001.md" in msg for msg in state["worker_rework_messages"])
    ...
    assert cycle_004["workflow_mode"] in ("discovery", "closure")  # score-based plateau may not trigger closure
