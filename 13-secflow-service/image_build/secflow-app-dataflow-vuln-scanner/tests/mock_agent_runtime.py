...
class MockAgentRuntime(BaseAgentRuntime):
    ...
    def _auto_response(
        ...
        *,
        ...
        if (
            "待验证的漏洞报告" in message_text
            or "真实性验证" in message_text
            or "待验证报告:" in message_text
            or "result file:" in message_text.lower()
        ):
            return json.dumps(
                {
                    "passed": True,
                    "verdict": "CONFIRMED",
                    "feedback": "Mock result review passed",
        ...
        if (
            "reflect" in message_text.lower()
            or "反思" in message_text
            or "系统性自审" in message_text
            or "深度自审" in message_text
        ):
