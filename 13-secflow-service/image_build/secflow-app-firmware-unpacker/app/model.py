...
DEFAULT_CONFIGS = [
    ...
    ("agent_run_timeout_seconds", "1800", "int", "PiRpcClient 单次调用的最大空闲时长（秒）；只在完全没有输出/事件时判定超时，-1=不限制"),
    ("agent_thinking_level", "", "string", "全局 pi agent thinking 级别：留空=使用 pi 默认行为；可选 off/minimal/low/medium/high/xhigh"),
    ("agent_timeout_retry_enabled", "true", "bool", "PiRpcClient 单次调用发生空闲超时后是否自动重试"),
    ("agent_timeout_max_retries", "20", "int", "PiRpcClient 单次调用发生空闲超时后的最大自动重试次数，-1=无限"),
    ...
    ("task_event_max_per_task", "10000", "int", "单个任务最多保留的事件数，超出后裁剪最旧事件，0=不自动裁剪"),
