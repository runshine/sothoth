...
class K8SService:
    ...
    def create_deployment(self, namespace: str, name: str, secmate_ng_id: str,
                         source_pvcs: List[Dict], output_pvcs: List[Dict],
                         custom_env: Dict[str, str] = None,
                         secmate_ng_env: Dict[str, Any] = None,
        ...
        Args:
            namespace: 命名空间
            name: Deployment名称
            secmate_ng_id: SecMate-NG ID
            source_pvcs: 源码PVC列表
            output_pvcs: 输出PVC列表
            custom_env: 自定义环境变量
            secmate_ng_env: SecMate-NG镜像环境变量配置
        ...
        try:
            ...
            # 构建SecMate-NG环境变量
            import secrets
            final_secmate_ng_env = {}
            cs_env_config = config.secmate_ng_env

            # 如果请求中提供了环境变量，使用请求中的值，否则使用配置中的值
            if secmate_ng_env:
                final_secmate_ng_env["PUID"] = str(secmate_ng_env.get("PUID", cs_env_config.get("PUID", 1000)))
                final_secmate_ng_env["PGID"] = str(secmate_ng_env.get("PGID", cs_env_config.get("PGID", 1000)))
                final_secmate_ng_env["TZ"] = secmate_ng_env.get("TZ", cs_env_config.get("TZ", "Asia/Shanghai"))
                final_secmate_ng_env["DEFAULT_WORKSPACE"] = secmate_ng_env.get("DEFAULT_WORKSPACE", cs_env_config.get("DEFAULT_WORKSPACE", "/config/workspace"))
                final_secmate_ng_env["PWA_APPNAME"] = secmate_ng_env.get("PWA_APPNAME", cs_env_config.get("PWA_APPNAME", "secmate-ng"))

                # 密码：优先使用请求中的，其次是配置中的，最后随机生成
                if secmate_ng_env.get("HASHED_PASSWORD"):
                    final_secmate_ng_env["HASHED_PASSWORD"] = secmate_ng_env["HASHED_PASSWORD"]
                elif secmate_ng_env.get("PASSWORD"):
                    final_secmate_ng_env["PASSWORD"] = secmate_ng_env["PASSWORD"]
                elif cs_env_config.get("HASHED_PASSWORD"):
                    final_secmate_ng_env["HASHED_PASSWORD"] = cs_env_config["HASHED_PASSWORD"]
                elif cs_env_config.get("PASSWORD"):
                    final_secmate_ng_env["PASSWORD"] = cs_env_config["PASSWORD"]
                else:
                    final_secmate_ng_env["PASSWORD"] = secrets.token_urlsafe(16)

                # Sudo密码
                if secmate_ng_env.get("SUDO_PASSWORD_HASH"):
                    final_secmate_ng_env["SUDO_PASSWORD_HASH"] = secmate_ng_env["SUDO_PASSWORD_HASH"]
                elif secmate_ng_env.get("SUDO_PASSWORD"):
                    final_secmate_ng_env["SUDO_PASSWORD"] = secmate_ng_env["SUDO_PASSWORD"]
                elif cs_env_config.get("SUDO_PASSWORD_HASH"):
                    final_secmate_ng_env["SUDO_PASSWORD_HASH"] = cs_env_config["SUDO_PASSWORD_HASH"]
                elif cs_env_config.get("SUDO_PASSWORD"):
                    final_secmate_ng_env["SUDO_PASSWORD"] = cs_env_config["SUDO_PASSWORD"]
                else:
                    final_secmate_ng_env["SUDO_PASSWORD"] = secrets.token_urlsafe(16)

                # 代理域名
                proxy_domain = secmate_ng_env.get("PROXY_DOMAIN", cs_env_config.get("PROXY_DOMAIN"))
                if proxy_domain:
                    final_secmate_ng_env["PROXY_DOMAIN"] = proxy_domain
            else:
                # 使用配置中的值
                final_secmate_ng_env["PUID"] = str(cs_env_config.get("PUID", 1000))
                final_secmate_ng_env["PGID"] = str(cs_env_config.get("PGID", 1000))
                final_secmate_ng_env["TZ"] = cs_env_config.get("TZ", "Asia/Shanghai")
                final_secmate_ng_env["DEFAULT_WORKSPACE"] = cs_env_config.get("DEFAULT_WORKSPACE", "/config/workspace")
                final_secmate_ng_env["PWA_APPNAME"] = cs_env_config.get("PWA_APPNAME", "secmate-ng")
                ...
                if cs_env_config.get("PROXY_DOMAIN"):
                    final_secmate_ng_env["PROXY_DOMAIN"] = cs_env_config["PROXY_DOMAIN"]

            # 构建完整的环境变量列表
            env = []

            # 1. SecMate-NG镜像环境变量（最重要，放最前面）
            for key, value in final_secmate_ng_env.items():
                env.append({"name": key, "value": value})

            # 2. 配置中的其他环境变量
            for key, value in config.env.items():
                # 避免覆盖SecMate-NG的特殊环境变量
                if key not in final_secmate_ng_env:
                    env.append({"name": key, "value": value})
            ...
            return True, final_secmate_ng_env
    ...
    def create_service(self, namespace: str, name: str, secmate_ng_id: str) -> Optional[str]:
        """创建Service"""
        try:
            config = self.config.secmate_ng
    ...
    def create_ingress(self, namespace: str, name: str, secmate_ng_id: str,
        ...
        try:
            ...
            # 生成访问域名
            host = f"{secmate_ng_id}.{config.base_domain}"
