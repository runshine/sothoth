...
class Case(Base):
    ...
    current_stage: Mapped[str] = mapped_column(String(32), default="receive", index=True)
...
class WorkflowRun(Base):
    ...
    case_id: Mapped[str] = mapped_column(ForeignKey("secflow_vuln_case.id"), index=True)
    workflow_definition_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True)
    current_stage: Mapped[str] = mapped_column(String(32), default="receive")
...
class EngineProjectConfig(Base):
    __tablename__ = "secflow_vuln_engine_project_config"
