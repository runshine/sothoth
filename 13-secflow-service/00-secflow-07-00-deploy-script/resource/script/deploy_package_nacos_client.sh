...
cat  <<EOF > ${ROOT_DIR}/usr/nacos/config.json
{
  ...
  "platform_agent_url": "http://secflow-platform-agent.secflow-ns.svc.cluster.local",
...
cat  <<EOF > ${ROOT_DIR}/service_config/99_nacos_client_service.json
{
  ...
  "start_cmd": "${ROOT_DIR}/usr/bin/python ${ROOT_DIR}/usr/nacos/nacos_client.py -c ${ROOT_DIR}/usr/nacos/config.json",
