...
if ! is_pid_file_running "${TTYD_ROOT_DIR}/run/ttyd.pid";then
  ...
  "${TTYD_ROOT_DIR}/../utils/ttyd" -p 11198 -w / -W "${ROOT_DIR}/docker/bin/docker_env.sh" >> "$TTYD_ROOT_DIR/log/ttyd.log" 2>&1 &
  echo "$!" > "$TTYD_ROOT_DIR/run/ttyd.pid"
  logger "start ttyd: \"${TTYD_ROOT_DIR}/../utils/ttyd\" -p 11198 -w / -W \"${ROOT_DIR}/docker/bin/docker_env.sh\" 2>&1 >> \"$TTYD_ROOT_DIR/log/ttyd.log\", pid:$(cat $TTYD_ROOT_DIR/run/ttyd.pid)"
else
