...
if [ "x${VPN_ONLY_NODE}" = "x" ];then
  ...
  bootstrap_utils_list="busybox bash nginx ttyd strace tcpdump openvpn curl socat ip rpcapd gdb hexdump xxd vim htop frida_server 7zz docker-compose jattach"
else
  echo "$(date): we are in VPN_ONLY_MODE"
  bootstrap_utils_list="busybox bash nginx ttyd strace tcpdump openvpn curl socat ip rpcapd gdb hexdump xxd vim htop"
fi
